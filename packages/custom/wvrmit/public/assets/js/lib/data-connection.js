...
(function() {
    ...
    function Signaler(root) {
        ...
        function onSocketMessage(data) {
            //console.log('Called back for data socket message: ' + JSON.stringify(data));
        ...
        this.broadcast = function(_config) {
            ...
            if (dataSocket.onDisconnect) dataSocket.onDisconnect().remove();
        };
        ...
        function leave() {
            if (dataSocket.remove) dataSocket.remove();
        ...
        if (!root.openSignalingChannel) {
            ...
            var dataSocket = new window.Firebase('https://' + (root.firebase || 'signaling') + '.firebaseIO.com/' + channel);
            dataSocket.on('child_added', function(snap) {
                var data = snap.val();
            ...
            /*var dataSocket = root.openSignalingChannel(function(message) {
                message = JSON.parse(message);
